...
function human(name, age, hairColor, race, alive) {
  ...
  const resuscitate = (target) => {
    if (target.alive == false) {
      health -= Math.floor(Math.random(1) * 6);
      target.health = 1;
      target.alive = true;
      result.textContent = `${name} resuscitated ${target.name} to 1 health and is now at ${health} health.`
    } else if (target.alive) {
      result.textContent = (`${target.name} is already alive`)
    }

  }

  const describe = () => {
    alert(`My name is ${name} and i have ${hairColor} hair and i am ${age} years old. I am ${race}.`)
  };

  const fight = (enemy) => {
    if (alive) {
      if (enemy.health > 0) {
        let randomNumber = Math.floor(Math.random(1) * 21);
      ...
      result.textContent = (`${name} is not alive.`)
    }
  }
